...
```
The function gX and gT return nonlinearity a two componenents vector. the first and second component are the nonlinearity from repulsive and attractive channels respectively (with unit $\mu$eV$\mu$m$^2$). The total nonlinearity is the sum of these two channels. We note that the code currently can only support the nonlinearity calculation for monolayer. Also, the trion nonlinearity calculation is only work for the wavefunction with $\lambda=\lambda'$. 
